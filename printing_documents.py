...
def create_front_sheet(village=False, no_print=False, nurses=False):
    ...
    logo = Image(r'J:\Quality Data\Data Technician\eCase Migration\ecase_automated\images/SAVLandscape.png')
    logo.anchor = 'A1'
    logo.width = 250
    logo.height = 40
    ...
    with open(rf'{constants.DOWNLOADS_DIR}\fs_Res.csv', newline='') as basic_info:
        ...
        try:
            ...
            if not any(substring in basic_data[1][9] for substring in doctors):
                respite = True
            month=['0','Jan','Feb','Mar','Apri','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec']
...
def village_birthdays(only_village=False,current=False):
    ...
    # birthdays, formatted for use by Liz Keene
        ...
        cakeimg = Image(rf'J:\Quality Data\Data Technician\eCase Migration\ecase_automated\images\birthdaycake.jpg')
        cakeimg.height = 100
        cakeimg.width = 100
        messageimg = Image(rf'J:\Quality Data\Data Technician\eCase Migration\ecase_automated\images\birthdayimage.jpg')
