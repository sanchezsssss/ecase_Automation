...
def resident_birthdays():
    ...
    tkinter.Button(birthday_window, text="Village Birthdays",
                   command=lambda: villageRensidentBTD()).grid(
        row=3, column=2)
    tkinter.Button(birthday_window, text="Quit",
                   command=lambda: birthday_window.destroy()).grid(
        row=5, column=2, pady=10)

def villageRensidentBTD():
    village_birthday_window = tkinter.Tk()
    village_birthday_window.wm_title("Village Birthdays")
    tkinter.Button(village_birthday_window,text = "Next Month",command\
        =lambda: resident_birthday_list(only_village=True)).grid(\
        row=2, column=2,padx=10, pady=5)

    tkinter.Button(village_birthday_window,text= "Current Month",command\
        =lambda: resident_birthday_list(only_village=True,current = True)).grid(\
        row=3, column=2)

    tkinter.Button(village_birthday_window, text="Quit",
                   command=lambda: village_birthday_window.destroy()).grid(
        row=5, column=2, pady=10)

    


def resident_birthday_list(only_village=False,current = False):
    """
    Opens a selenium browser with ecase_login, and downloads eCase reports
    ...
    if file_available(rf'{constants.OUTPUTS_DIR}\Resident Birthdays\ResidentBirthdays.xlsx'):
        ...
        time.sleep(4)
        ecase_driver.quit()
        printing_documents.village_birthdays(only_village=only_village,current=current)
